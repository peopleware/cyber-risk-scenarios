...
export const RiskScenario = Joi.object({
  ...
  references: Joi.object({
    violates: Joi.array().items(Violations.optional()).required(),
    ISO27001A: Joi.array().items(ISO27001AControls.optional()).required(),
    stride: Joi.array().items(STRIDE.optional()).required(),
    linddun: Joi.array().items(LINDDUN.optional()).required()
  }).required()
})
